...
@router.post("/form-fill", response_model=FormResponse)
...
@router.post("/update-form-fill", response_model=FormUpdateResponse)
...
def download_pdf(request: DownloadRequest):
    form_service = FormService()
    result = form_service.download_pdf(request)
    return result
